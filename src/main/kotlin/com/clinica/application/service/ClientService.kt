...
class ClientService(private val clientRepository: ClientRepository) : ClientServicePort {
    ...
    override fun create(request: ClientRequest): ClientResponse {
        require(!clientRepository.existsByEmail(request.email)) {
            "A client with email '${request.email}' already exists"
        }
        ...
        return clientRepository.save(clientEntity).toResponse()
    }
    ...
    private fun ClientEntity.toResponse() = ClientResponse(
        id = id,
        firstName = firstName,
        lastName = lastName,
        email = email,
