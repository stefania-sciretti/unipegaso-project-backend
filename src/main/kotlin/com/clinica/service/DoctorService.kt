...
class DoctorService(private val doctorRepository: DoctorRepository) {
    ...
    fun create(request: DoctorRequest): DoctorResponse {
        if (doctorRepository.existsByLicenseNumber(request.licenseNumber)) {
            throw IllegalArgumentException("A doctor with license number '${request.licenseNumber}' already exists")
        }
        val doctor = Doctor(
    ...
    fun delete(id: Long) {
        if (!doctorRepository.existsById(id)) {
            throw NoSuchElementException("Doctor not found with id: $id")
        }
